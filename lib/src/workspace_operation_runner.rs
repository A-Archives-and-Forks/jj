...
/// A [`WorkspaceOperationRunner`]  encapsulates an [`WorkspaceEnvironment`], a [`ReadonlyRepo`]
/// and a [`Workspace`]. It can be used to create snapshots and deals with exporting jj-internal
/// state to Git if the functionality is compiled in. Library users should wrap it for their
/// use-cases, like `jj-cli` does.
///
