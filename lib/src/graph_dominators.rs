    ...
    N: Clone + Eq + Hash + PartialEq,
{
        ...
        NI: IntoIterator<Item = N>,
    {
        ...
        let target_ids: Option<Vec<Index>> = target_set
            ...
            .collect();
        if target_ids.as_ref().is_none_or(|ids| ids.is_empty()) {
            return None;
        }
        ...
        // start_node_id is always num_nodes-1 because of the way we construct the
        // postorder.
        ...
        let lca = Self::find_set_lca(&target_ids, &idom);

        // 7: Map internal ID back to generic type T
        Some(id_to_node[lca].clone())
    }
    ...
    fn find_set_lca(targets: &[Index], idom: &[Option<Index>]) -> Index {
        assert!(!targets.is_empty());
        let mut lca = targets[0];
        for &node in &targets[1..] {
            lca = Self::intersect(lca, node, idom);
        }
        lca
    }
}
...
mod tests {
    ...
    fn closest_common_dominator(
        ...
        let target_set_string = target_set.iter().map(|&n| n.to_string()).collect_vec();
